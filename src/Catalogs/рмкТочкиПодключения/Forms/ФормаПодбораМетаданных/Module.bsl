...
Процедура ОкончаниеЗаписиФайлаМетаданных(Результат, ДополнительныеСвойства) Экспорт
	...
	Если Результат <> Истина Тогда
		ОбщегоНазначенияКлиентПереопределяемый.СообщитьПользователю("Не удалось выгрузить метаданные!");
	КонецЕсли;			
...
Процедура ОкончаниеЧтенияФайлаМетаданных(ДополнительныеСвойства) Экспорт
	...
	Исключение
		...
		ОбщегоНазначенияКлиентПереопределяемый.СообщитьПользователю("Не удалось прочитать файл");
		ОбщегоНазначенияКлиентПереопределяемый.СообщитьПользователю(ОбработкаОшибок.ПодробноеПредставлениеОшибки(
			ИнформацияОбОшибке()));
